...
from pycaption import Caption, CaptionList, CaptionSet, SCCReader, CaptionReadNoCaptions, CaptionNode
...
class TestSCCReader(ReaderTestingMixIn):
    ...
    def test_positioning(self, sample_scc_multiple_positioning):
        ...
        actual_positioning = [caption_.layout_info.origin.serialized() for caption_ in captions.get_captions("en-US")]
    ...
    def test_default_positioning_when_no_positioning_is_specified(self, sample_no_positioning_at_all_scc):
        caption_set = SCCReader().read(sample_no_positioning_at_all_scc)

        actual_caption_layouts = [caption.layout_info.serialized() for caption in caption_set.get_captions("en-US")]
    ...
    def test_timing_is_properly_set_on_split_captions(
        ...
        caption_set = SCCReader().read(sample_scc_produces_captions_with_start_and_end_time_the_same)
        expected_timings = [
        ...
        actual_timings = [(c_.format_start(), c_.format_end()) for c_ in caption_set.get_captions("en-US")]

        assert expected_timings == actual_timings

    def test_skip_extended_characters_ascii_duplicate(self, sample_scc_with_extended_characters):
    ...
    def test_skip_duplicate_special_characters(self, sample_scc_duplicate_special_characters):
    ...
    def test_line_too_long(self, sample_scc_with_line_too_long):
        ...
        assert exc_info.value.args[0].startswith("32 character limit for caption cue in scc file.")
        assert (
...
class TestCoverageOnly:
    ...
    def test_multiple_formats(self, sample_scc_multiple_formats):
        ...
        text_lines = [node.content for caption in captions for node in caption.nodes if node.type_ == CaptionNode.TEXT]
...
class TestTimingCorrectingCaptionList:
    ...
    def test_last_caption_zero_end_time_is_corrected(self, sample_scc_no_explicit_end_to_last_caption):
